...
class NutritionFactsLabel(BaseModel):
    serving_size: Optional[str] = Field(
        None, description="The serving size, typically represented as a quantity with unit"
    )
    ...
    additional_nutrients: Optional[Dict[str, NutrientInfo]] = Field(
        None, description="Additional nutrients not covered by standard fields"
    )

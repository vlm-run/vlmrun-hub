...
class DemographicInformation(BaseModel):
    ...
    date_of_birth: date | None = Field(
        None, description="Date of birth of the patient."
    )
...
class MedicalHistory(BaseModel):
    conditions: list[str] | None = Field(
        None, description="List of known medical conditions."
    )
    medications: list[str] | None = Field(
        None, description="List of medications the patient is currently taking."
    )
    allergies: list[str] | None = Field(
        None, description="List of allergies the patient has."
    )
    surgeries: list[str] | None = Field(
...
class InsuranceDetails(BaseModel):
    ...
    coverage_start_date: date | None = Field(
...
class ConsentForm(BaseModel):
    ...
    signed_date: date | None = Field(
        None, description="Date when the consent form was signed."
    )
...
class PatientImage(BaseModel):
    ...
    image_type: str | None = Field(
        None, description="Type of image, e.g., 'profile photo', 'X-ray'."
    )

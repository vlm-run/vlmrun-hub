...
class ProductImage(BaseModel):
    ...
    is_primary: bool | None = Field(
        None, description="Indicates if this is the main display image"
    )
    dimensions: dict[str, int] | None = Field(
        None, description="Image dimensions in pixels (width, height)"
    )
...
class ProductFeature(BaseModel):
    ...
    is_key_feature: bool | None = Field(
        None, description="Indicates if it's a key selling point"
    )
...
class StyleDescriptor(BaseModel):
    ...
    category: str | None = Field(
        None, description="Style category (e.g., Modern, Casual, Formal)"
    )
    details: str | None = Field(None, description="Additional style details")
    color_palette: list[str] | None = Field(
        None, description="List of colors used in the product"
    )
...
class KeyBenefit(BaseModel):
    ...
    description: str | None = Field(
        None, description="Detailed explanation of the benefit"
    )
    impact_score: int | None = Field(
        None, description="Priority score (1-10) for the benefit"
    )
...
class PricingInfo(BaseModel):
    ...
    currency: str | None = Field(
        None, description="Three-letter currency code (e.g., USD)"
    )
...
class SEOInfo(BaseModel):
    ...
    keywords: list[str] | None = Field(
        None, description="SEO keywords for search visibility"
    )
    meta_description: str | None = Field(
        None, description="SEO-optimized product description"
    )
...
class ShippingInfo(BaseModel):
    ...
    dimensions: dict[str, float] | None = Field(
        None, description="Product dimensions in centimeters"
    )
    delivery_estimate: str | None = Field(
        None, description="Estimated delivery timeframe"
    )
...
class EcommerceProductCaption(BaseModel):
    ...
    product_id: str | None = Field(
        None, description="Unique identifier for the product"
    )
    product_name: str | None = Field(None, description="Name of the product")
    product_images: list[ProductImage] | None = Field(
        None, description="List of product images"
    )
    product_features: list[ProductFeature] | None = Field(
        None, description="List of product features"
    )
    style_descriptors: list[StyleDescriptor] | None = Field(
        None, description="Style and design information"
    )
    key_benefits: list[KeyBenefit] | None = Field(
        None, description="Core product benefits"
    )
    pricing_info: PricingInfo | None = Field(
        None, description="Pricing and availability details"
    )
    ...
    shipping_info: ShippingInfo | None = Field(
        None, description="Shipping specifications"
    )
    product_category: ProductCategory | None = Field(
        None, description="Product categorization"
    )
    generated_caption: str | None = Field(
        None, description="AI-generated marketing caption"
    )
    additional_info: dict[str, str] | None = Field(
        None, description="Additional product metadata"
    )

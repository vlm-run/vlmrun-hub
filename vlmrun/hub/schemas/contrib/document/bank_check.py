...
class BankCheck(BaseModel):
    check_number: Optional[str] = Field(
        None, description="Check number, typically printed in the top right corner of the check"
    )
    payment_date: Optional[str] = Field(None, description="Date written on the check")
    payee: Optional[str] = Field(
        None, description="Name of the person or entity to whom the check is payable (Pay to the order of)"
    )
    ...
    micr_line: Optional[str] = Field(
        None,
        description="MICR (Magnetic Ink Character Recognition) line at the bottom of the check containing routing and account numbers",
    )
